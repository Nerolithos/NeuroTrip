...
export type OpticalApproximation = {
  defocusStrength: number
  anisotropyStrength: number
  axisRad: number
  focusMismatchD: number
  refractiveRegime: 'myopia' | 'hyperopia' | 'emmetropia'
  modelNote: string
}

export const computeOpticalApproximation = (params: RefractionParams): OpticalApproximation => {
  ...
  let refractiveRegime: OpticalApproximation['refractiveRegime'] = 'emmetropia'
  ...
  const defocusStrength = clamp(
    normalizedMismatch * (0.82 + 0.14 * pupilGain) + normalizedCylinder * 0.08,
    0,
    1,
  )
  ...
  return {
    ...
    refractiveRegime,
    modelNote:
      'Perceptual educational approximation: diopter controls are mapped to screen-space blur and anisotropy, not clinical retinal image formation.',
  }
}
