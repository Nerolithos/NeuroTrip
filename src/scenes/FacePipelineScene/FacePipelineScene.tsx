...
import { requestEmojiMatch, requestEmojiReason, resolveChatapConfig, type ChatapConfig } from './emojiMatcher'
...
export const FacePipelineScene = () => {
  ...
  const triggerEmojiMatch = async () => {
    ...
    setEmojiStatus(isZh ? '表情识别中，请稍候...' : 'Classifying expression, please wait...')
    setEmojiCognition(isZh ? '认知描述将由独立通道生成...' : 'Perception narrative will be generated by an independent channel...')

    try {
      const nextEmoji = await requestEmojiMatch({ config, imageDataUrl, isZh })
      let nextReason: string | null = null

      // Reason generation runs independently so it cannot bias emoji classification.
      nextReason = await requestEmojiReason({
        config,
        imageDataUrl,
        isZh,
        emojiHint: nextEmoji,
      ...
      if (nextEmoji) {
        ...
        setEmojiCognition((nextReason || '').trim() || buildPerceptionNarrativeFallback(isZh, nextEmoji))
      } else {
  ...
  useEffect(() => {
    ...
    const prepareEmojiMatch = () => {
      ...
      const config = resolveChatapConfig({
        ...
        reasonModel: (env.VITE_CHATAP_REASON_MODEL || '').trim(),
        reasonFallbackModel: (env.VITE_CHATAP_REASON_MODEL_FALLBACK || '').trim(),
