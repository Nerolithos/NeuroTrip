...
export type NextWordPrediction = {
  token: string
  probability: number
}
...
export type LanguageCommands = {
  dark: boolean
  fall: boolean
  silence: boolean
}

export type SemanticUniverse = {
  anchorWord: string
  key: string
  title: string
  description: string
...
const transitionHints: Record<string, string[]> = {
...
const semanticUniverses: Array<
  Omit<SemanticUniverse, 'anchorWord' | 'confidence' | 'hints'> & { anchorWord: string; cues: string[] }
> = [
  {
    anchorWord: 'bank',
    key: 'bank-river',
...
export const tokenizeInput = (text: string): string[] => {
  return (text || '')
    .toLowerCase()
    .replace(/[\u2019']/g, ' ')
...
export const wordToPseudoPhonemes = (word: string): string[] => {
...
export const buildLexicalCompetition = (token: string): LexicalCandidate[] => {
...
export const detectSemanticAnomaly = (tokens: string[]): boolean => {
  return tokens.some((token) => anomalyLexicon.has(token))
}

const normalizePredictions = (predictions: NextWordPrediction[]): NextWordPrediction[] => {
...
export const predictNextWords = (tokens: string[]): NextWordPrediction[] => {
...
const bucketForToken = (token: string) => {
...
export const buildSemanticClusters = (tokens: string[]): SemanticCluster[] => {
...
const includesAny = (tokens: string[], lexicon: string[]) => {
  for (let i = 0; i < lexicon.length; i += 1) {
    if (tokens.includes(lexicon[i]!)) return true
  }
  return false
}

export const detectLanguageCommands = (text: string): LanguageCommands => {
...
const queryTermsForLabel = (label: string): string[] => {
...
export const scoreLabelAffinity = (label: string, semanticTags: string[]): number => {
  const terms = queryTermsForLabel(label)
  const tags = semanticTags.map((tag) => (tag || '').toLowerCase().trim()).filter(Boolean)
  if (!terms.length || !tags.length) return 0
...
export const resolveSemanticUniverse = (input: string | string[]): SemanticUniverse => {
